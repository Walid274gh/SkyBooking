...
public class ManagerHelper {
    ...
    // ==================== 🆕 CONVERSIONS POUR MODULE HÔTEL ====================
    
    /**
     * 🆕 Convertir Document MongoDB en Hotel CORBA
     */
    public static Hotel documentToHotel(Document doc) {
        Hotel hotel = new Hotel();
        hotel.hotelId = doc.getString("hotelId");
        hotel.hotelName = doc.getString("hotelName");
        hotel.city = doc.getString("city");
        hotel.address = doc.getString("address");
        hotel.starRating = doc.getInteger("starRating", 3);
        hotel.description = doc.getString("description");
        hotel.pricePerNight = doc.getDouble("pricePerNight");
        hotel.availableRooms = doc.getInteger("availableRooms", 0);
        hotel.imageUrl = doc.getString("imageUrl");
        hotel.amenities = doc.getString("amenities");
        hotel.reviewScore = doc.getDouble("reviewScore");
        hotel.reviewCount = doc.getInteger("reviewCount", 0);
        return hotel;
    }
    
    /**
     * 🆕 Convertir Document MongoDB en HotelReservation CORBA
     */
    public static HotelReservation documentToHotelReservation(Document doc) {
        HotelReservation res = new HotelReservation();
        res.hotelReservationId = doc.getString("hotelReservationId");
        res.customerId = doc.getString("customerId");
        res.hotelId = doc.getString("hotelId");
        res.hotelName = doc.getString("hotelName");
        res.checkInDate = doc.getString("checkInDate");
        res.checkOutDate = doc.getString("checkOutDate");
        res.numberOfNights = doc.getInteger("numberOfNights", 0);
        res.numberOfRooms = doc.getInteger("numberOfRooms", 0);
        res.originalPrice = doc.getDouble("originalPrice");
        res.discountPercentage = doc.getDouble("discountPercentage");
        res.finalPrice = doc.getDouble("finalPrice");
        res.status = doc.getString("status");
        res.reservationDate = doc.getString("reservationDate");
        res.flightReservationId = doc.getString("flightReservationId");
        res.hasFlightDiscount = doc.getBoolean("hasFlightDiscount", false);
        return res;
    }
}