...
public class FlightManagerImpl extends FlightManagerPOA {
    ...
    System.out.println("🌍 Initialisation OTA Mondiale - Génération optimisée");
    ...
    for (int day = 0; day < totalDays; day++) {
        ...
        if ((day + 1) % 15 == 0) {
            System.out.println("📅 Génération: " + (day + 1) + "/" + totalDays + 
                             " jours - " + allFlights.size() + " vols créés");
        }
    }
    ...
    // ✅ ROUTES PRIORITAIRES - Générées TOUS LES JOURS
    Map<String, String[]> guaranteedRoutes = new HashMap<String, String[]>() {{
        put("Alger", new String[]{
            "Paris", "Marseille", "Lyon", "Londres", "Madrid", "Rome",
            "Istanbul", "Dubaï", "Doha", "Le Caire", "Tunis", "Casablanca"
        });
        put("Oran", new String[]{
            "Paris", "Marseille", "Lyon", "Madrid", "Barcelone",
            "Istanbul", "Tunis", "Casablanca"
        });
        put("Constantine", new String[]{
            "Paris", "Marseille", "Lyon", "Tunis"
        });
        put("Annaba", new String[]{
            "Paris", "Marseille", "Tunis"
        });
    }};
    
    int flightCounter = dayOffset * 1000 + 50000;
    
    // Génération des routes garanties pour chaque hub
    for (String hub : algerianHubs) {
        String[] destinations = guaranteedRoutes.get(hub);
        
        for (String destination : destinations) {
            // ✅ Générer 1-2 vols par jour pour chaque route
            int flightsPerRoute = hub.equals("Alger") ? 2 : 1;
            
            for (int flightNum = 0; flightNum < flightsPerRoute; flightNum++) {
                String flightId = "IT" + String.format("%06d", flightCounter++);
                
                // Horaires variés
                int departureHour = flightNum == 0 ? 
                    (6 + random.nextInt(4)) : (14 + random.nextInt(6));
                ...
                int returnDepartureHour = (departureHour + 6 + random.nextInt(4)) % 24;
                String returnDepartureTime = String.format("%02d:%02d", 
                    returnDepartureHour, random.nextInt(4) * 15);
