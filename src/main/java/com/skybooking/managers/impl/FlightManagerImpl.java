...
public class FlightManagerImpl extends FlightManagerPOA {
    ...
    private List<Document> generateInternationalFlights(String date, int dayOffset) {
        ...
        for (String hub : algerianHubs) {
            // Destinations par région depuis chaque hub
            for (Map.Entry<String, String[]> region : INTERNATIONAL_DESTINATIONS.entrySet()) {
                String[] destinations = region.getValue();
                
                // Sélection de destinations du jour (rotation)
                int destinationsPerDay = hub.equals("Alger") ? 8 : 
                                        hub.equals("Oran") ? 4 : 3;
                
                for (int d = 0; d < destinationsPerDay && d < destinations.length; d++) {
                    int destIndex = (dayOffset + d) % destinations.length;
                    String destination = destinations[destIndex];
